...
// ignore: unused_element
  Failure<T> failure<T>({@required T error}) {
    return Failure<T>(
      error: error,
    );
  }
...
mixin _$ApiResult<T> {
  ...
  TResult when<TResult extends Object>({
    ...
    @required TResult failure(T error),
  });
  @optionalTypeArgs
...
class _$FailureCopyWithImpl<T, $Res> extends _$ApiResultCopyWithImpl<T, $Res>
  ...
  $Res call({
    ...
    return _then(Failure<T>(
      error: error == freezed ? _value.error : error as T,
    ));
  }
}
...
abstract class Failure<T> implements ApiResult<T> {
  const factory Failure({@required T error}) = _$Failure<T>;
